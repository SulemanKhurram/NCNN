...
from .BBBdistributions import Normal, Normalout, distribution_selector,FixedNormal
from torch.nn.modules.utils import _pair
import config as cf

cuda = torch.cuda.is_available() and cf.allow_cuda
...
class _ConvNdCustomPriors(nn.Module):
    """
    ...
    def __init__(self, prior_tensor, in_channels, out_channels, kernel_size, stride,
                 padding, dilation, output_padding, groups, p_logvar_init=-3, p_pi=1.0, q_logvar_init=-5):
        super(_ConvNdCustomPriors, self).__init__()
        ...
        self.qw_mean = Parameter(prior_tensor)
        ...
        # self.pw = distribution_selector(mu=0.0, logvar=p_logvar_init, pi=p_pi)
        # self.pw = FixedNormal(self.pw_mean, self.p_logvar_init)
...
class _ConvNd(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride,
        ...
        self.qw_logvar = Parameter(tensor)
        # self.qb_mean = Parameter(torch.Tensor(out_channels))
        # self.qb_logvar = Parameter(torch.Tensor(out_channels))
...
class BBBConv1d(_ConvNd):
    ...
    def convprobforward(self, input):
        ...
        # local reparameterization trick for convolutional layer
        conv_qw_mean = F.conv1d(input=input, weight=self.qw_mean, stride=self.stride, padding=self.padding,dilation=self.dilation, groups=self.groups)
        conv_qw_si = torch.sqrt(1e-8 + F.conv1d(input=input.pow(2), weight=torch.exp(self.log_alpha)*self.qw_mean.pow(2),
                                                stride=self.stride, padding=self.padding, dilation=self.dilation, groups=self.groups))
...
class BBBConv2dCustomPriors(_ConvNdCustomPriors):
    def __init__(self,prior_weights, in_channels, out_channels, kernel_size, stride=1,
                 padding=0, dilation=1, groups=1):
        ...
        super(BBBConv2dCustomPriors, self).__init__(prior_weights, in_channels, out_channels, kernel_size, stride, padding, dilation, _pair(0), groups)
...
class BBBLinearFactorial_CW(nn.Module):
    """
    Describes a Linear fully connected Bayesian layer with
    a distribution over each of the weights and biases
    in the layer.
    """
    def __init__(self,prior_weights, in_features, out_features, p_logvar_init=-3, p_pi=1.0, q_logvar_init=-5):
        ...
        super(BBBLinearFactorial_CW, self).__init__()
        ...
        # Approximate posterior weights...
        self.qw_mean = Parameter(prior_weights)
        ...
        # prior model
        self.pw_mean = Parameter(prior_weights)
        # (does not have any trainable parameters so we use fixed normal or fixed mixture normal distributions)
        # self.pw = distribution_selector(mu=0.0, logvar=p_logvar_init, pi=p_pi)
        self.pw = FixedNormal(self.pw_mean, self.p_logvar_init)
        # self.pw = distribution_selector(mu=0.0, logvar=p_logvar_init, pi=p_pi)
