...
from utils.Models.HybridModels import Hybrid_5Conv3FC,Hybrid_5Conv1FC,Hybrid_6Conv3FC
import numpy as np
from utilities_general import eval


parser = argparse.ArgumentParser(description='PyTorch Hybrid Model Training')
...
base_path = validate_dir(cf.hybrid_base_folder+'/')
...
batch_size, optim_type =  cf.batch_size, cf.optim_type
...
def getNetwork(args):
    if (args.net_type == 'alexnet_5Conv1FC'):
        net = Hybrid_5Conv1FC(num_classes,inputs)

    elif (args.net_type == 'alexnet_5Conv3FC'):
        net = Hybrid_5Conv3FC(num_classes,inputs)
    elif (args.net_type == 'alexnet_6Conv3FC'):
        net = Hybrid_6Conv3FC(num_classes,inputs)
    else:
...
print('| Getting Feature extractor from checkpoint...')
net, file_name = getNetwork(args)
checkpoint = torch.load(cf.hybrid_feature_extractor)
feature_ext = checkpoint['net']
weight_to_be_used = []
biases_to_be_used = []
for mod in feature_ext.features.children():
    if isinstance(mod, nn.modules.conv.Conv2d):
        weight_to_be_used.append(mod.weight.data)
        biases_to_be_used.append(mod.bias.data)

fc_weight_to_be_used = []
for mod in feature_ext.classifier.children():
    if isinstance(mod, nn.modules.linear.Linear):
        fc_weight_to_be_used.append(mod.weight.data)

i = 0
for mod in net.features.children():
    if isinstance(mod, nn.modules.conv.Conv2d):
        mod.weight.data = weight_to_be_used[i]
        mod.bias.data = biases_to_be_used[i]
        i+=1
...
logfile = os.path.join(experiment_path + 'diagnostics_Hybrid{}_{}_{}.txt'.format(args.net_type, args.dataset, cf.num_samples))
...
def test(epoch):
    ...
    torch.save(state, save_ck_path + file_name + str(epoch) + '.t7')
